import { ChatAltIcon, ChevronLeftIcon, ChevronRightIcon, ExternalLinkIcon, HeartIcon} from '@heroicons/react/outline'
import React from 'react'
import {useParams, Link} from "react-router-dom"
import {UserAuth} from "../context/AuthContext"
import { db } from '../firebase';
import { doc, setDoc, runTransaction, updateDoc, arrayUnion} from "firebase/firestore";
...
function Singlecourse({courses}) {
    ...
    const [viewComment, setViewComment] = React.useState(false)
      ...
      const [commentData, setCommentData] = React.useState(
        ...
        const addComment = async (event) => {
            ...
            const thisCourse = courses?.filter(course => course.docName === courseName)
  ...
  return (
    ...
    {courses?.filter(course => course.docName === courseName).map((course) => (
